...
**6‑5.** **Important:** After finishing configuration, **hold Button A for 3 seconds again** to exit Config Mode. After that, the device redraws the current month’s calendar. The device cannot enter sleep mode while in Config Mode, which drains the battery.  
<br clear="right" />  

+ **About Holidays**  
  + Holidays data are written in YAML (indent with 2 or 4 spaces). Only the Keys are used, the Values(after `:`) are not used.  
